...
function mixinMigration(PostgreSQL) {
  ...
  PostgreSQL.prototype.buildColumnDefinition = function(model, propName) {
    ...
    // Read column type property
    const columnMetadata = this.columnMetadata(model, propName);
    ...
    if (prop.generated) {
      // For types with prefix BIG should be used BIGSERIAL
      if (colType && colType.startsWith('BIG'))
        return 'BIGSERIAL';
      return 'SERIAL';
    }
