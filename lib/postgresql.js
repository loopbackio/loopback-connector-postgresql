...
exports.initialize = function initializeDataSource(dataSource, callback) {
  ...
  if (dbSettings.parseInt8) {
    postgresql.defaults.parseInt8 = true;
  }
  if (dbSettings.parseFloat) {
    // float4
    postgresql.types.setTypeParser(700, function(val) {
      return parseFloat(val);
    });
    // float8
    postgresql.types.setTypeParser(701, function(val) {
      return parseFloat(val);
    });
    // numeric
...
PostgreSQL.prototype.dropTable = function(model, cb) {
  this.execute('DROP TABLE IF EXISTS ' + this.tableEscaped(model) + ' CASCADE', cb);
};
...
PostgreSQL.prototype.connect = function(callback) {
  var self = this;
  function handleResponse(client, done, callback, err) {
    self.client = client;
  ...
  self.pg.connect(self.clientConfig, function(err, client, done) {
    if (err) {
      if (err && err.message.match(/database ".*?" does not exist/i) && self.settings.createDatabase) {
        var config = JSON.parse(JSON.stringify(self.clientConfig));
        delete config.database;

        self.pg.connect(config, function(err, client, done) {
          client.query('CREATE DATABASE "' + self.settings.database + '"', function(err, res) {
            if (err) {
              handleResponse(client, done, callback, err);
              return;
            }
            self.pg.connect(self.clientConfig, function(err, client, done) {
              handleResponse(client, done, callback, err);
