...
PostgreSQL.prototype.executeSQL = function(sql, params, options, callback) {
  ...
  function executeWithConnection(connection, autocommit, done) {
    if (self.settings.vertica) {
      sql = prepare(sql, params, autocommit);
      params = [];
    }
    connection.query(sql, params, function(err, data) {
      ...
      if (data) {
        switch (data.command) {
          ...
          case 'UPDATE':
            if (self.settings.vertica) {
              // Vertica returns a row with OUTPUT set to number of rows
              // affected in string form
              result = {count: Number(data.rows[0].OUTPUT)}
            } else {
  ...
  function prepare(sql, params, autocommit) {
    if (Array.isArray(params)) {
      for (var i = 0, n = params.length; i < n; i++) {
        var val = pgUtils.prepareValue(params[i]);
  ...
  if (transaction && transaction.connection &&
    ...
    executeWithConnection(transaction.connection, false, null);
  } else {
    self.pg.connect(function(err, connection, done) {
      if (err) return callback(err);
      executeWithConnection(connection, true, done);
    });
  }
};

PostgreSQL.prototype.buildInsertReturning = function(model, data, options) {
  if (this.settings.vertica) {
...
PostgreSQL.prototype.getInsertedId = function(model, info) {
  if (this.settings.vertica) {
    var idColName = 'LAST_INSERT_ID';
    var idValue;
    if (info && info[1]) {
      idValue = info[1][idColName];
    }
    return idValue;
  }
