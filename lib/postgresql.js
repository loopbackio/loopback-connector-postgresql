...
function PostgreSQL(postgresql, settings) {
  ...
  //TODO (gdingle): create setting for postgis or check for it dynamically
  //  if (this.settings.enableGeoIndexing === true) {
  //   PostgreSQL.prototype.buildNearFilter = buildNearFilter;
  // } else {
  //   PostgreSQL.prototype.buildNearFilter = undefined;
  // }
}
...
PostgreSQL.prototype.buildExpression = function(columnName, operator,
  ...
  switch (operator) {
    ...
    case 'near':
      //TODO (gdingle): WHERE ST_Distance(col, Point($1, $2)) < $3
    default:
...
/**
 * See https://github.com/strongloop/loopback-connector-mongodb/blob/a44aec12572111d86d622c4083e9dbed78e739b0/lib/mongodb.js#L759
 */
function buildNearFilter(query, near) {
  ...
  //TODO (gdingle): dynamically override buildOrderBy to emit SQL
  // 'ORDER BY ST_Distance('+coordinates.lat+', '+coordinates.lng+')'

  //TODO (gdingle): check for other ORDER BY clauses which may conflict
  // ...prepend? append or error?

  if (near.maxDistance) {
    //TODO (gdingle): query.where[near.key] = ...

    //TODO (gdingle):
    //use postgresql default unit 'meters' rather than 'miles'
    // see convertToMeters in mongodb connector
  }
};
