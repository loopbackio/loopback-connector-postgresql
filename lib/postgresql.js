...
PostgreSQL.prototype.buildSelect = function(model, filter) {
  ...
  if (filter) {
    if (filter.where) {
      const joinStmt = this.buildJoin(model, filter.where);
...
PostgreSQL.prototype.buildJoin = function (model, where) {
  let modelDefinition, candidateProperty, candidateRelation, candidateTable;
  const innerJoins = [];
  ...
  for (const key in where) {
    ...
    candidateRelation = splitedKey[0]
    candidateProperty = splitedKey[1]
    if (!modelDefinition) {
      modelDefinition = this.getModelDefinition(model)
    }
    if (!modelDefinition) {
      debug('No definition for model %s', model);
      continue
    }
    if (!(candidateRelation in modelDefinition.settings.__relations__)) {
      debug('No relation for model %s', model);
      continue
    }
    const relation = modelDefinition.settings.__relations__[candidateRelation]
    if (relation.type !== 'belongsTo' && relation.type !== 'hasOne') {
      ddebug('Invalid relation type for model %s for inner join', model);
      continue
    }
    ...
    p = targetDefinition.properties[candidateProperty]
    candidateTable = this.tableEscaped(target.modelName)
    innerJoins.push({
      type: relation.type,
  ...
  for (const innerJoin of innerJoins) {
    ...
    joinStmt.sql += `INNER JOIN ${innerJoin.tableName} AS "_${innerJoin.relationName}" ON ${self.columnEscaped(model, key1)}::uuid = ${self.columnEscaped(innerJoin.model,key2,'_')}`
  }
...
PostgreSQL.prototype._buildWhere = function(model, where) {
  ...
  for (const key in where) {
    ...
    if (p == null) {
      ...
      const relation = modelDefinition.settings.__relations__[candidateRelation]
      if (relation.type !== 'belongsTo' && relation.type !== 'hasOne') {
        debug('Invalid relation type for model %s for inner join', model);
        continue
      }
      const target = relation.target()
      const targetDefinition = this.getModelDefinition(target.modelName)
    ...
    const columnName = innerJoin ? `${self.columnEscaped(targetModel, candidateProperty, '_')}` : self.columnEscaped(model, key);
