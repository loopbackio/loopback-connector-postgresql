...
PostgreSQL.prototype.create = function (model, data, callback) {
  ...
  var dataIdNames = idNames.filter(function(key) { return data[key] != null;});
  var fieldNames = Object.keys(data).filter(function(key) {
    return idNames.indexOf(key);
  });

  var sql = [];
  sql.push('INSERT INTO ', self.tableEscaped(model), ' ', self.toFields(model, data, true));
  sql.push(' RETURNING ');
  sql.push(idNames.map(function(key) { return self.columnEscaped(model, key)}).join(','));

  var queryparams = [];
  fieldNames.forEach(function(key) {
    queryparams.push(data[key]);
  });
  dataIdNames.forEach(function(key) {
...
PostgreSQL.prototype.updateOrCreate = function (model, data, callback) {
  var self = this;
  var idNames = self.idNames(model).map(function(key) { return self.columnEscaped(model, key); });
  var fieldNames = Object.keys(data).map(function(key) { return self.columnEscaped(model, key); }).filter(function(key) {
    return idNames.indexOf(key);
  });
  var query = [];
  ...
  query.push(" WHERE ");
  query.push(idNames.map(function(key, i) {
    return ((i > 0)?' AND ': ' ') + key + '=$' + (fieldNames.length+i+1) }).join(','));
  query.push(' RETURNING ', idNames.join(','), ')');
  query.push(', insert_outcome AS (INSERT INTO ', self.tableEscaped(model), ' ');
  query.push(self.toFields(model, data, true));
  query.push(' WHERE NOT EXISTS (SELECT * FROM update_outcome) RETURNING ', idNames.join(','), ')');
  ...
  self.query(query.join(''), queryparams, function(err, data) {
    callback(err, data[0]);
  });
};
...
PostgreSQL.prototype.save = function (model, data, callback) {
  ...
  idNames.forEach(function (id, i) {
    sql.push((i > 0) ? ' AND ' : ' ', self.idColumnEscaped(model), ' = $', (fieldNames.length+i+1));
  });
...
PostgreSQL.prototype.toFields = function (model, data, forCreate) {
  ...
  if (forCreate) {
    query.push('(');
    query.push(fieldNames.map(function(key) { return self.columnEscaped(model, key);}).join(','));
    if(dataIdNames.length > 0) {
      if (fieldNames.length > 0) { query.push(',');}
      query.push(dataIdNames.map(function(key) { return self.columnEscaped(model, key);}).join(','));
    }
    query.push(') SELECT ');
    for(var i= 1, len= fieldNames.length + dataIdNames.length; i<=len; i++) {
      query.push('$', i);
      if(i != len) { query.push(',');}
    }
    query.push(' ');
  } else {
    query.push(fieldNames.map(function(key, i) { return self.columnEscaped(model, key) + "=$" + (i+1)}).join(','));
  }
  return query.join('');
};
