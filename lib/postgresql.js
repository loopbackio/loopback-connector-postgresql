...
PostgreSQL.prototype.buildFields = function(model, data, excludeIds) {
  ...
  var fields = this._buildFieldsForKeys(model, data, keys, excludeIds);
  for (var i = 0; i < fields.properties.length; i++) {
    if (fields.properties[i].postgresql.dataType === 'composite') {
      let value = fields.columnValues[i];
      let name = fields.names[i];
      fields.columnValues.splice(i, 1);
      fields.names.splice(i, 1);
      let subFields = this.buildFields(fields.properties[i].type.modelName, data[name.substr(1).slice(0, -1)]);
      for (var j = 0; j < subFields.properties.length; j++) {
        fields.names.push(name + '.' + subFields.names[j]);
...
PostgreSQL.prototype.columnEscaped = function(model, property) {
  if (isNested(property)) {
    ...
    if (this.getPropertyDefinition(model, property).postgresql.dataType === 'composite') {
