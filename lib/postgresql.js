...
PostgreSQL.prototype.buildFields = function(model, data, excludeIds) {
  var keys = Object.keys(data);
  var props = this.getModelDefinition(model).properties;
  var nativeFields = [];
  var compositeFields = [];
  keys.forEach(function(field) {
    if (props[field].postgresql.composite) {
      compositeFields.push(field);
    }
    else {
      nativeFields.push(field);
    }
  });
  var fields = this._buildFieldsForKeys(model, data, nativeFields);
  for (var i = 0; i < compositeFields.length; i++) {
    var field = compositeFields[i];
    var subFields = this.buildFields(props[field].type.modelName, data[field]);
    for (var j = 0; j < subFields.properties.length; j++) {
      fields.names.push(this.escapeName(field) + '.' + subFields.names[j]);
      fields.columnValues.push(subFields.columnValues[j]);
    }
  }
  return fields;
}
...
PostgreSQL.prototype.columnEscaped = function(model, property) {
  if (isNested(property)) {
    ...
    if (this.getPropertyDefinition(model, propertyParts[0]).postgresql.composite) {
      return '(' + this.escapeName(propertyParts[0]) + ').' + this.escapeName(propertyParts[1]); 
    }
    ...
    var propertyName = this.escapeName(this.column(model, property));
    if (this.getPropertyDefinition(model, property).postgresql && this.getPropertyDefinition(model, property).postgresql.composite) {
      propertyName = 'row_to_json(' + propertyName + ') AS ' + property;
    }
    return propertyName;
  }
};
