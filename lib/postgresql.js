...
PostgreSQL.prototype.buildSelect = function (model, filter) {
  ...
  if (filter) {
    if (filter.where) {
      const { joinStmt, innerWhere } = this.buildJoin(model, filter.where);
      filter.innerWhere = innerWhere
      selectStmt.merge(joinStmt);
    }
    if (filter.where) {
      const whereStmt = this.buildWhere(model, filter.where, filter.innerWhere);
 ...
 */
PostgreSQL.prototype.columnEscaped = function (model, property, prefix = '') {
  if (isNested(property)) {
    ...
    return this.escapeName(prefix + this.table(model)) + '.' + this.escapeName(this.column(model, property));
  }
};
...
PostgreSQL.prototype.buildJoin = function (model, where) {
  let candidateProperty, candidateRelation;
  const innerJoins = [];
  const innerWhere = {};
  if (!where) {
  ...
  for (const key in where) {
    ...
    candidateRelations = splitedKey[0]
    ...
    for (let i = 0; i < candidateRelations.length; i++) {
      ...
      const relation = modelDefinition.settings.__relations__[candidateRelation]
      ...
      if (!(candidateProperty in targetDefinition.properties)) {
        debug('Unknown property %s is skipped for model %s', candidateProperty, target.modelName);
        break;
      }
      let alreadyJoined = false
      for (const innerJoin of innerJoins) {
        if (innerJoin.model === target.modelName && innerJoin.parentModel === parentModel) {
          alreadyJoined = true
          innerWhere[key] = {
            key: `${innerJoin.prefix}${innerJoin.relationName}`,
            value: where[key]
          }
        }
      }
      if (!alreadyJoined) {
        innerJoins.push({
          ...
          relationName: candidateRelation,
          tableName: this.tableEscaped(target.modelName),
  ...
  for (const innerJoin of innerJoins) {
    const key1 = innerJoin.type === 'belongsTo' ? innerJoin.keyFrom : innerJoin.keyTo
    const key2 = innerJoin.type === 'belongsTo' ? innerJoin.keyTo : innerJoin.keyFrom
    joinStmt.sql += `INNER JOIN ${innerJoin.tableName} AS "${innerJoin.prefix}${innerJoin.relationName}" ON ${self.columnEscaped(innerJoin.parentModel, key1)}::uuid = ${self.columnEscaped(innerJoin.model,key2,innerJoin.prefix)}`
  }

  return { joinStmt, innerWhere }
}
 ...
 */
PostgreSQL.prototype.buildWhere = function (model, where, innerWhere) {
  const whereClause = this._buildWhere(model, where, innerWhere);
  if (whereClause.sql) {
 ...
 */
PostgreSQL.prototype._buildWhere = function (model, where, innerWhere) {
  let columnValue, sqlExp, candidateProperty, candidateRelation, modelDefinition, targetModel;
  if (!where) {
    return new ParameterizedSQL('');
  }
  if (typeof where !== 'object' || Array.isArray(where)) {
  ...
  for (const key in where) {
    ...
    let expression = innerJoin ? inneWhere[key].value : where[key];
    const columnName = innerJoin ? `${self.columnEscaped(innerWhere[key].model, innerWhere[key].property, innerWhere[key].prefix)}` : self.columnEscaped(model, key);
