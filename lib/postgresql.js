...
PostgreSQL.prototype.escapeName = function (name) {
  // we should not escape names containing '->' i.e. json fields: dbJSONfield->'objField'
...
PostgreSQL.prototype._buildWhere = function (model, conds) {
  ...
  if (typeof conds === 'string') {
    ...
    Object.keys(conds).forEach(function (key) {
      ...
      var isJsonField = (key.indexOf('->') != -1);
      if (props[key] || isJsonField ) {
        var filterValue;
        if(isJsonField) {
          // All values shoud be quoted in JSON query. Even numbers.
          filterValue = self.toDatabase({type:{name:'String'}}, conds[key]);
        } else {
          filterValue = self.toDatabase(props[key], conds[key]);
        }
        if (filterValue === 'NULL') {
